...
| Modelo 2 (agricultura) como extensión del Modelo 1, priorizado tras Modelo 1 (PIB) | Mitigación del riesgo de tiempo limitado señalado en la propuesta | Implementado en Fase 06 (`src/model2_agri.py`, reutiliza `panel_base.py`/`simulate.py`/`interpret.py` sin modificar; selector "Modelo activo" en el dashboard) |
